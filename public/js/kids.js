...
function getActions(item_key, listoffset) {
  var data = {};
  data.zone_id = settings.zoneID;
  data.options = { item_key: item_key };
  ...
  $.ajax({
    ...
    url: "/roonapi/goRefreshBrowse",
    success: function(payload) {
      showActions(payload, settings.zoneID);
    }
  });
}

function showActions(payload, zone_id) {
  console.log('function showActions(payload, zone_id)' );
  ...
  if (payload !== undefined) {
    ...
    //console.log(items);
    $(".actionList").html('');
    $("#overlayActionList").show();
    for (var x in items) {
      if (x>2) break;
      $(".actionList").append(
        '<button type="button" class="buttonOverlay buttonActionId" id="button-' +
          titleToID(items[x].title) +
...
function selectAction(item_key, listoffset) {

  //Hide overlay
  $("#overlayActionList").hide();
  $(".actionList").html('');
...
function showData(payload, zone_id) {
  console.log('function showData(payload, zone_id)' );
  console.log(payload);
  $("#buttonRefresh")
    .html(getSVG("refresh"))
  ...
  var list = payload.data.list;
i
  // If level 0 let's find Playlists and jump right to it
  if (list.level==0) {
    for (var i in items) {
      if (items[i].title == "Playlists") {
        goList(items[i].item_key);
        break;
      }
    }
    //stop function!
  ...
  if (items !== null) {
    ...
    for (var x in items) {
      ...
      if (items[x].input_prompt) {
        ...
        if (list.level>0 || items[x].title == "Playlists") {
          //List item buttons (songs, actions, etc)
          var click_act = "";
          if (items[x].hint == "action_list") {
            //show simplified action list
            click_act = 'getActions';
          } else {
            click_act = 'goList';
          }

          // Show number for easier kids recall "Track #"
          var itemNumeral = "";
          if(list.subtitle != null) {
              var subItems = parseInt(list.subtitle.split(" ")[0]);
              if (items.length > subItems ) {
                itemNumeral = (x>0)?x+". ":"";
              }
          } else {
            itemNumeral = (list.level>0)?(parseInt(x) +1)+". ":"";;
          }

          //console.log(items[x].title + " = " + titleToID(items[x].title))
          html +=
            '<button type="button" id="' +titleToID(items[x].title)+'" class="itemTileItem" onclick="' + click_act +'(\'' +
            items[x].item_key +
            "')\">";
          html += itemNumeral + items[x].title;
          html += "</button>";
        }
      }
      if (html != "") {
